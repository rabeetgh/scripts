...
if __name__ == "__main__":
    ...
    # bands
    run = Vasprun("./bands/vasprun.xml", parse_projected_eigen=True)
    bands = run.get_band_structure("./bands/KPOINTS",
                                   line_mode=True,
    ...
    emin -= bands.efermi + 1
    emax -= bands.efermi - 1
    ax1.set_ylim(emin, 6.0)
    ax2.set_ylim(emin, 6.0)
    ...
    # fermi level at 0
    ax1.hlines(y=0, xmin=0, xmax=len(bands.kpoints), color="k", lw=1)
    ...
    ax2.set_xlim(1e-6, 3)
    ax2.set_xticklabels([])
    ax2.hlines(y=0, xmin=0, xmax=8, color="k", lw=1)
    ...
    # plt.show()
    plt.savefig(sys.argv[0].strip(".py") + ".eps", format="eps")
